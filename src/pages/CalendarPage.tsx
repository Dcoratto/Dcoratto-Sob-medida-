...
export const CalendarPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => { unsubQuotes(); unsubClients(); unsubCondominiums(); unsubManualEvents(); };
  }, []);
  ...
  const events = useMemo(() => {
    ...
    manualEvents.forEach((manualEvent) => {
      ...
      if (!date) return;
      list.push({id: `manual-${manualEvent.id}`, date, type: 'manual', title: manualEvent.title, description: manualEvent.description, clientId: manualEvent.clientId, clientName: manualEvent.clientName, city: manualEvent.city, status: 'Evento manual'});
    });
    return list;
  }, [clients, manualEvents, quotes]);

  const days = useMemo(() => Array.from({length: 42}, (_, index) => { const day = startOfMonthGrid(baseDate); day.setDate(day.getDate() + index); return day; }), [baseDate]);
  const eventByDay = useMemo(() => {
    const map = new Map<string, CalendarEvent[]>();
    events.forEach((event) => map.set(keyOf(event.date), [...(map.get(keyOf(event.date)) || []), event]));
    return map;
  }, [events]);
  ...
  const upcomingEvents = useMemo(() => events.map((event) => ({event, daysLeft: daysLeftFromToday(event.date)})).filter((item) => item.daysLeft >= 0).sort((a, b) => a.daysLeft - b.daysLeft).slice(0, 12), [events, today]);
  ...
  return (
    <div className="space-y-6">
      ...
      {restrictions.length > 0 && <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><div className="flex items-center gap-2 text-amber-800 font-bold"><AlertTriangle className="w-5 h-5" />Alertas de restrição</div><div className="mt-3 space-y-2">{restrictions.slice(0, 8).map(({event, condominium, reason}) => <div key={`${event.id}-${reason}`} className="text-sm text-amber-900">{eventLabel(event.type)} de {event.clientName} em {event.date.toLocaleDateString('pt-BR')} no {condominium.name}: {reason}.</div>)}</div></section>}
      {deadlineAlerts.length > 0 && <section className="rounded-2xl border border-rose-200 bg-rose-50 p-4"><div className="flex items-center gap-2 text-rose-800 font-bold"><AlertTriangle className="w-5 h-5" />Alertas de prazo</div><div className="mt-3 space-y-2">{deadlineAlerts.slice(0, 8).map(({event, daysLeft, level}) => <div key={`${event.id}-deadline`} className={cn('text-sm font-semibold', level === 'maximo' ?'text-rose-900' : 'text-amber-800')}>{level === 'maximo' ?'ALERTA MÁXIMO' : 'ALERTA DE PRAZO'}: {event.clientName || event.title} em {event.date.toLocaleDateString('pt-BR')} ({daysLeft === 0 ?'hoje' : `faltam ${daysLeft} dia${daysLeft > 1 ?'s' : ''}`})</div>)}</div></section>}

      <section className="rounded-2xl border border-slate-200 bg-white p-4"><div className="text-slate-800 font-bold">Contagem regressiva</div><div className="mt-3 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2">{upcomingEvents.length === 0 && <div className="rounded-xl bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-400">Nenhum evento futuro cadastrado.</div>}{upcomingEvents.map(({event, daysLeft}) => <button key={`${event.id}-countdown`} type="button" onClick={() => setSelectedEvent(event)} className={cn('rounded-xl px-3 py-2 text-left transition-all', daysLeft <= 2 ?'bg-rose-50 hover:bg-rose-100' : daysLeft <= 7 ?'bg-amber-50 hover:bg-amber-100' : 'bg-slate-50 hover:bg-slate-100')}><div className="text-xs font-bold uppercase tracking-wider text-slate-500">{eventLabel(event.type)} · {event.clientName || event.title}</div>{daysLeft <= 2 && <div className="mt-1 text-[10px] font-black uppercase tracking-wider text-rose-700">Alerta máximo</div>}{daysLeft > 2 && daysLeft <= 7 && <div className="mt-1 text-[10px] font-black uppercase tracking-wider text-amber-700">Alerta de prazo</div>}<div className="mt-1 text-sm font-bold text-slate-800">{daysLeft === 0 ?'É hoje' : `Daqui a ${daysLeft} dia${daysLeft > 1 ?'s' : ''}`}</div><div className="text-xs text-slate-500">{event.date.toLocaleDateString('pt-BR')}</div></button>)}</div></section>

      <section className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden"><div className="grid grid-cols-7 border-b border-slate-100 text-center text-xs font-bold uppercase tracking-widest text-slate-400">{['Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb', 'Dom'].map((label) => <div key={label} className="py-3">{label}</div>)}</div><div className="grid grid-cols-7">{days.map((day) => { const key = keyOf(day); const isCurrentMonth = day.getMonth() === baseDate.getMonth(); const isToday = keyOf(day) === keyOf(today); const nationalHoliday = getHolidayInfo(day).national; const cityHolidays = altoTieteCities.map((city) => ({city, name: getHolidayInfo(day, city).city})).filter((item) => item.name); const dayEvents = eventByDay.get(key) || []; return <div key={key} className={cn('min-h-[132px] border-r border-b border-slate-100 p-2', !isCurrentMonth && 'bg-slate-50/70', isToday && 'bg-brand-primary/5 ring-1 ring-brand-primary/30')}><div className="flex items-start justify-between gap-2"><div className={cn('text-xs font-bold', isToday ?'text-brand-primary' : isCurrentMonth ?'text-slate-700' : 'text-slate-400')}>{day.getDate()}{isToday ?' · Hoje' : ''}</div><button type="button" onClick={() => openCreateModal(day)} className="rounded p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600" title="Adicionar evento"><Plus className="w-3.5 h-3.5" /></button></div>{nationalHoliday && <div className="mt-1 rounded bg-rose-50 px-1.5 py-0.5 text-[10px] font-bold text-rose-700">{nationalHoliday}</div>}{cityHolidays.slice(0, 2).map((holiday) => <div key={`${key}-${holiday.city}`} className="mt-1 rounded bg-blue-50 px-1.5 py-0.5 text-[10px] font-bold text-blue-700">{holiday.city}: {holiday.name}</div>)}{cityHolidays.length > 2 && <div className="mt-1 text-[10px] font-bold text-blue-500">+{cityHolidays.length - 2} feriados municipais</div>}<div className="mt-2 space-y-1">{dayEvents.slice(0, 3).map((event) => { const daysLeft = daysLeftFromToday(event.date); return <button key={event.id} type="button" onClick={() => setSelectedEvent(event)} className={cn('w-full rounded-lg px-2 py-1.5 text-left text-[11px] font-semibold transition-all', daysLeft >= 0 && daysLeft <= 2 ?'bg-rose-50 text-rose-700 hover:bg-rose-100' : daysLeft >= 0 && daysLeft <= 7 ?'bg-amber-50 text-amber-700 hover:bg-amber-100' : 'bg-slate-50 text-slate-600 hover:bg-slate-100')}>{eventLabel(event.type)} · {event.clientName || event.title}</button>;})}{dayEvents.length > 3 && <div className="text-[10px] font-bold text-slate-400">+{dayEvents.length - 3} mais</div>}</div></div>;})}</div></section>

      {selectedEvent && <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4"><div className="w-full max-w-md rounded-3xl bg-white border border-slate-100 shadow-2xl p-6"><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-bold uppercase tracking-widest text-slate-400">{eventLabel(selectedEvent.type)}</div><h3 className="mt-1 text-xl font-display font-bold text-slate-900">{selectedEvent.clientName || selectedEvent.title}</h3><div className="mt-1 text-sm font-semibold text-slate-500">{selectedEvent.date.toLocaleDateString('pt-BR')} · {selectedEvent.status || 'Sem status'}</div></div><button type="button" onClick={() => setSelectedEvent(null)} className="rounded-full p-2 text-slate-400 hover:bg-slate-100"><X className="w-5 h-5" /></button></div>{selectedEvent.type === 'manual' ?<div className="mt-5 rounded-2xl bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700">{selectedEvent.description?.trim() || 'Sem descrição.'}</div> : <div className="mt-5 space-y-3"><div className="rounded-2xl bg-slate-50 px-4 py-3"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-400"><Phone className="w-4 h-4" />Telefone</div><div className="mt-1 text-sm font-semibold text-slate-800">{selectedClient?.phone || 'Não informado'}</div></div><div className="rounded-2xl bg-slate-50 px-4 py-3"><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-slate-400"><MapPin className="w-4 h-4" />Endereço</div><div className="mt-1 text-sm font-semibold text-slate-800">{selectedClient?.address || 'Não informado'}</div></div></div>}</div></div>}

      {showCreateModal && <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4"><div className="w-full max-w-md rounded-3xl bg-white border border-slate-100 shadow-2xl p-6"><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-bold uppercase tracking-widest text-slate-400">Novo evento</div><h3 className="mt-1 text-xl font-display font-bold text-slate-900">Adicionar no calendário</h3></div><button type="button" onClick={() => setShowCreateModal(false)} className="rounded-full p-2 text-slate-400 hover:bg-slate-100"><X className="w-5 h-5" /></button></div><form className="mt-5 space-y-3" onSubmit={handleCreateEvent}><div><label className="text-xs font-bold uppercase tracking-widest text-slate-500">Cliente vinculado (opcional)</label><select value={newEventClientId} onChange={(event) => setNewEventClientId(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-brand-primary"><option value="">Sem cliente vinculado</option>{clients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select></div><div><label className="text-xs font-bold uppercase tracking-widest text-slate-500">Título</label><input value={newEventTitle} onChange={(event) => setNewEventTitle(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-brand-primary" placeholder="Ex: visita técnica" /></div><div><label className="text-xs font-bold uppercase tracking-widest text-slate-500">Data</label><input type="date" value={newEventDate} onChange={(event) => setNewEventDate(event.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-brand-primary" required /></div><div><label className="text-xs font-bold uppercase tracking-widest text-slate-500">Descrição (opcional)</label><textarea value={newEventDescription} onChange={(event) => setNewEventDescription(event.target.value)} rows={3} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-brand-primary" placeholder="Detalhes do que será feito" /></div><button type="submit" disabled={isSavingEvent} className="w-full rounded-xl bg-brand-primary px-3 py-2 text-sm font-bold text-white hover:brightness-105 disabled:opacity-70">{isSavingEvent ?'Salvando...' : 'Salvar evento'}</button></form></div></div>}
    </div>
  );
};
