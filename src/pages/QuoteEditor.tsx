  ...
  const handleSave = async () => {
    ...
    try {
      if (id) {
        ...
        await applyQuoteInventoryByStatusTransition(createdRef.id, 'OrÃƒÂ§amento', status, quoteData);
        await logSystemEvent({
          type: 'quote_created',
              ...
              {pieceAreaDetails.map(({piece, totals, material}) => (
                <div key={piece.id} className="space-y-1">
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1 opacity-80">
                    ...
                    <span className="col-span-2">Final: {formatNumber(totals.totalArea, 4)} MÂ² | {material?.name || 'Sem material'}</span>
                  </div>
                </div>
              ))}
            </div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex bg-slate-100 p-1 rounded-xl">
                            ...
                            onClick={() => updatePiece(piece.id, { sculptedSink: { ...(piece.sculptedSink || {
                              drainType: 'VÃƒÂ¡lvula oculta', quantity: 1, width: 0, depth: 0, height: 0, unit: 'cm'
                            }), active: true } as any })}
                      ...
                      <div className="bg-slate-50/50 border border-slate-100 rounded-3xl p-6 space-y-6 animate-in slide-in-from-top-2">
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                          <div className="space-y-1">
                            ...
                            <select 
                              value={piece.sculptedSink.drainType || 'VÃƒÂ¡lvula oculta'}
                            ...
                            >
                              <option value="VÃƒÂ¡lvula oculta">VÃƒÂ¡lvula oculta</option>
                              ...
                              return (
                                <>
                                  ...
                                  <div className="space-y-0.5">
                                    ...
                                    <div className="text-slate-900 font-mono font-bold">{piece.sculptedSink.drainType || 'VÃƒÂ¡lvula oculta'}</div>
                                  </div>
                                  <div className="space-y-0.5">
