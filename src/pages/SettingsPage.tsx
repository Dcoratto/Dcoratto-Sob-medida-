...
import { Save, Plus, Trash2, Building, Phone, Mail, Calculator, CreditCard, Scissors, Pencil } from 'lucide-react';
              ...
              <div key={condo.id} className="rounded-2xl bg-slate-50 p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                <div>
                  ...
                  <div className="text-xs text-slate-500">
                    {condo.city} · {(condo.addressMode || 'street') === 'lot' ? 'Quadra e lote' : 'Rua e número'} · {condo.workStartHour}-{condo.workEndHour} · Dias: {condo.allowedWeekdays.map((item) => ['Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sab', 'Dom'][item]).join(', ')}
                  </div>
                </div>
