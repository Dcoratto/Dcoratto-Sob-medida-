...
begin
  with piece_source as (
    select
      ...
      case
        when jsonb_typeof(piece -> 'presentationBaseValue') = 'number' then (piece ->> 'presentationBaseValue')::numeric
        when nullif(piece ->> 'presentationBaseValue', '') is not null then (piece ->> 'presentationBaseValue')::numeric
        ...
        when jsonb_typeof(piece -> 'value') = 'number' then (piece ->> 'value')::numeric
        when nullif(piece ->> 'value', '') is not null then (piece ->> 'value')::numeric
        else null
      end as piece_base_value,
  ...
  piece_distribution_source as (
    select
      piece_enriched.*,
      greatest(coalesce(piece_base_value, 0), 0) as piece_base_value_safe,
      round(greatest(coalesce(piece_base_value, 0), 0) * 100)::bigint as piece_base_cents,
      count(*) over () as piece_count,
      sum(round(greatest(coalesce(piece_base_value, 0), 0) * 100)::bigint) over () as piece_base_total_cents,
      greatest(round(coalesce(p_quote.total_price, 0) * 100)::bigint, 0) as target_total_cents
    from piece_enriched
  ),
  piece_distribution_basis as (
    select
      piece_distribution_source.*,
      (target_total_cents - piece_base_total_cents) as diff_cents,
      abs(target_total_cents - piece_base_total_cents) as abs_diff_cents,
      case when (target_total_cents - piece_base_total_cents) >= 0 then 1 else -1 end as diff_sign,
      case
        when piece_count <= 1 then 'single-piece'
        when target_total_cents = piece_base_total_cents then 'unchanged'
        when piece_base_total_cents > 0 then 'proportional'
        else 'equal-zero-base'
      end as allocation_mode,
      case
        when piece_base_total_cents > 0 then piece_base_cents
        else 1::bigint
      end as allocation_weight
    from piece_distribution_source
  ),
  piece_distribution_calculated as (
    select
      piece_distribution_basis.*,
      sum(allocation_weight) over () as allocation_weight_total,
      case
        when piece_count <= 1 or diff_cents = 0 then 0::bigint
        else ((abs_diff_cents * allocation_weight) / nullif(sum(allocation_weight) over (), 0))::bigint
      end as allocated_base_cents,
      case
        when piece_count <= 1 or diff_cents = 0 then 0::bigint
        else (abs_diff_cents * allocation_weight) % nullif(sum(allocation_weight) over (), 0)
      end as remainder_numerator
    from piece_distribution_basis
  ),
  piece_distribution_ranked as (
    select
      piece_distribution_calculated.*,
      (abs_diff_cents - sum(allocated_base_cents) over ()) as residual_cents,
      row_number() over (
        order by
          remainder_numerator desc,
          allocation_weight desc,
          ordinality asc
      ) as residual_rank
    from piece_distribution_calculated
  ),
  piece_distribution_final as (
    select
      piece_distribution_ranked.*,
      case
        when piece_count <= 1 then diff_cents
        when diff_cents = 0 then 0::bigint
        else (
          allocated_base_cents
          + case when residual_rank <= residual_cents then 1 else 0 end
        ) * diff_sign
      end as allocated_adjustment_cents,
      case
        when piece_count <= 1 then target_total_cents
        when diff_cents = 0 then piece_base_cents
        else piece_base_cents + (
          allocated_base_cents
          + case when residual_rank <= residual_cents then 1 else 0 end
        ) * diff_sign
      end as piece_final_cents
    from piece_distribution_ranked
  ),
  material_source as (
    select distinct on (
  ...
  select
      ...
      jsonb_agg(
        jsonb_strip_nulls(
          jsonb_build_object(
            ...
            'area', piece_area,
            'value', (piece_final_cents::numeric / 100),
            'baseValue', (piece_base_cents::numeric / 100),
            'allocatedAdjustmentValue', (allocated_adjustment_cents::numeric / 100),
            'presentationFinalValue', (piece_final_cents::numeric / 100),
    ...
    coalesce(sum(piece_base_cents), 0)::numeric / 100
  into v_piece_rows, v_material_rows, v_piece_subtotal
  from piece_distribution_final;
